...
from source.structure.forms import UploadSentAAudioForm, ButtonAddWordForm, ButtonAddGrammarForm, ButtonDeleteForm, \
    BackButtonForm, RightSentForm, UploadVideoForm, RightSentBForm, WrongSentForm
from source.static.media_handler import add_to_task_video, add_to_task_sent_A_audio

task_20_bp = Blueprint('task_20_bp', __name__, url_prefix='/task_20_puzzle_lang_from_char', template_folder='templates')


@flask_sijax.route(task_20_bp, '<int:task_id>/', methods=["GET", "POST"])
def render(task_id):
    ...
    right_sent_lst = task.right_sentences['sent_lang_B']
    add_right_sent_form = RightSentBForm()
    if add_right_sent_form.validate_on_submit() and add_right_sent_form.submit_B.data:
        right_sent = add_right_sent_form.sent_lang_B.data
        right_sent_lst = task.right_sentences['sent_lang_B']
        right_sent_lst.append(right_sent)
        task.right_sentences['sent_lang_B'] = right_sent_lst

        db.session.commit()
        flash('right word was added successfully')
    ...
    if add_wrong_sent_form.validate_on_submit() and add_wrong_sent_form.add_wrong_sent.data:
        ...
        wrong_sent_lst = task.wrong_sentences['sent_lang']
        wrong_sent_lst.append(wrong_sent_lang)
        task.wrong_sentences['sent_lang'] = wrong_sent_lst

        db.session.commit()
        flash('wrong word was added successfully')
        return redirect(url_for('structure.render_task', task_id=task.id))
    ...
    return render_template('tasks/20_puzzle_lang_from_char.html',
                           ...
                           add_right_sent_form=add_right_sent_form,
                           right_sent_lst=right_sent_lst,
