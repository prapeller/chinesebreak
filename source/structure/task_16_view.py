...
from source.structure.forms import UploadSentAAudioForm, UploadSentBAudioForm, ButtonAddWordForm, ButtonAddGrammarForm, \
    ButtonDeleteForm, BackButtonForm, RightSentAForm, RightSentBForm, WrongSentForm, UploadVideoForm
from source.static.media_handler import add_to_task_video, add_to_task_sent_A_audio, add_to_task_sent_B_audio

task_16_bp = Blueprint('task_16_bp', __name__, url_prefix='/task_16_dialog_A_char_from_video',
                       template_folder='templates')


@flask_sijax.route(task_16_bp, '<int:task_id>/', methods=["GET", "POST"])
def render(task_id):
    task = Task.query.filter_by(id=task_id).first()
    task_type = TaskType.query.filter_by(id=task.task_type_id).first()

    video_name = Media.query.filter_by(id=task.media['sent_video_id'][0]).first().name if task.media[
        'sent_video_id'] else 'None'
    sent_A_video_form = UploadVideoForm()
    if sent_A_video_form.validate_on_submit() and sent_A_video_form.video.data:
        task_video = add_to_task_video(task=task, file=sent_A_video_form.video.data)
    ...
    right_sent_A_form.sent_pinyin_A.data = task.right_sentences.get('sent_pinyin_A')[0] if task.right_sentences.get(
        'sent_pinyin_A') else ''
    right_sent_A_form.sent_char_A.data = task.right_sentences.get('sent_char_A')[0] if task.right_sentences.get(
        'sent_char_A') else ''
    right_sent_A_form.sent_lang_A.data = task.right_sentences.get('sent_lang_A')[0] if task.right_sentences.get(
        'sent_lang_A') else ''
    right_sent_A_form.sent_lit_A.data = task.right_sentences.get('sent_lit_A')[0] if task.right_sentences.get(
        'sent_lit_A') else ''

    right_sent_B_form.sent_pinyin_B.data = task.right_sentences.get('sent_pinyin_B')[0] if task.right_sentences.get(
        'sent_pinyin_B') else ''
    right_sent_B_form.sent_char_B.data = task.right_sentences.get('sent_char_B')[0] if task.right_sentences.get(
        'sent_char_B') else ''
    right_sent_B_form.sent_lang_B.data = task.right_sentences.get('sent_lang_B')[0] if task.right_sentences.get(
        'sent_lang_B') else ''
    right_sent_B_form.sent_lit_B.data = task.right_sentences.get('sent_lit_B')[0] if task.right_sentences.get(
        'sent_lit_B') else ''
    ...
    return render_template('tasks/16_dialog_B_char_from_video.html',
                           task=task, task_type=task_type,
                           sent_A_video_form=sent_A_video_form,
