...
task_21_bp = Blueprint('task_21_bp', __name__, url_prefix='/task_21_puzzle_char_from_video', template_folder='templates')


@flask_sijax.route(task_21_bp, '<int:task_id>/', methods=["GET", "POST"])
def render(task_id):
    ...
    if upload_video_form.validate_on_submit() and upload_video_form.video.data:
        task_video = add_to_task_video(task=task, file=upload_video_form.video.data)
    ...
    wrong_task_words_id_lst = task.elements.get('words_id_wrong')
    wrong_task_words = [Word.query.filter_by(id=id).first() for id in wrong_task_words_id_lst if
                        Word.query.filter_by(id=id).first()]
    ...
    search_wrong_words_val = request.args.get('search_wrong_words_key')
    if search_wrong_words_val:
        wrong_words = Word.query.filter(Word.char.contains(search_wrong_words_val) | Word.pinyin.contains(
            search_wrong_words_val) | Word.lang.contains(search_wrong_words_val))
    else:
        wrong_words = Word.query.all()
    ...
    return render_template('tasks/21_puzzle_char_from_video.html',
                           ...
                           wrong_task_words=wrong_task_words,
                           grammars=grammars,
