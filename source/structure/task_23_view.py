...
task_23_bp = Blueprint('task_23_bp', __name__, url_prefix='/task_23_grammar_choose_from_video', template_folder='templates')


@flask_sijax.route(task_23_bp, 'task_<int:task_id>/', methods=["GET", "POST"])
def render(task_id):
    ...
    task_grammar = Grammar.query.filter_by(id=grammar_id).first()
    wrong_grammars = Grammar.query.filter(Grammar.id.in_(task.elements['grammars_id_wrong']))
    ...
    return render_template('tasks/23_grammar_choose_from_video.html',
                           ...
                           grammars=grammars,
                           wrong_grammars=wrong_grammars,
                           # task_grammars_id_list=task_grammars_id_list,
                           task_grammar=task_grammar,
