...
from source.structure.forms import UploadSentAAudioForm, ButtonAddWordForm, ButtonAddGrammarForm, ButtonDeleteForm, \
    BackButtonForm, RightSentForm, WrongSentForm, UploadVideoForm
from source.static.media_handler import add_to_task_video, add_to_task_sent_A_audio

task_9_bp = Blueprint('task_9_bp', __name__, url_prefix='/9_sent_lang_from_video', template_folder='templates')


@flask_sijax.route(task_9_bp, '<int:task_id>/', methods=["GET", "POST"])
def render(task_id):
